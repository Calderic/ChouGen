...
export async function createSmokingRecord(
  ...
  try {
    const res = await fetch('/api/smoking-records', {
      ...
      body: JSON.stringify({
        packId,
        smokedAt: smokedAt ? smokedAt.toISOString() : undefined,
        forceUnlock,
      }),
    });
    ...
    if (!res.ok) {
      return {
        ...
        errorCode: res.status === 403 ? 'LOCKED' : undefined,
      };
    }
    return {
      success: true,
      data: json.data as SmokingRecord,
      is_violation: json.is_violation,
    };
  } catch (e) {
